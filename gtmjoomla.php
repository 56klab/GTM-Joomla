 ...
 * @version		0.0.1
 * @package		GtmJoomla
 * @subpackage  System.GtmJoomla
 * @copyright	Simone Bussoni, www.quantility.it
 * @license		GNU GPL
 */
...
class plgSystemGtmJoomla extends JPlugin {
	...
	public function onContentPrepare($context, &$row, &$params, $page = 0)
	{
		...
		if ($addScrollTracker == 'on') {
			...
			$document->addScript('plugins/system/gtmjoomla/js/scroll-tracker.js');
			$jsContent = 'jQuery(document).ready(function(){jQuery.contentIdPlugin.contentIdValue(\''.$scrollTrackerContentId.'\')});';
	...
	function onAfterRender() {
		...
		$gtmjoomla_js_container_code = "\n<!-- Google Tag Manager for Joomla JS container -->
<script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],
j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src=
'//www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
})(window,document,'script','".$dataLayerName."','".$container_id."');</script>
<!-- End Google Tag Manager for Joomla JS container -->";
		...
		if ($addDataLayer == 'on') {
			...
			$buffer = preg_replace ("/(<head(?!er).*>)/i", "$1"."\n<script>".$dataLayerCode."</script>".$gtmjoomla_js_container_code, $buffer, 1);
		}
		else {
			$buffer = preg_replace ("/(<head(?!er).*>)/i", "$1".$gtmjoomla_js_container_code, $buffer, 1);
		}
		// String containing the iframe code to be placed after the <body> tag
		$gtmjoomla_iframe_container_code = "\n<!-- Google Tag Manager for Joomla noscript-iframe -->
<noscript><iframe src=\"https://www.googletagmanager.com/ns.html?id=".$container_id."\"
height=\"0\" width=\"0\" style=\"display:none;visibility:hidden\"></iframe></noscript>
<!-- End Google Tag Manager for Joomla noscript-iframe -->";
		
		// update to limit = 1 to add tag to only the first <body.*> tag
		$buffer = preg_replace ("/(<body.*?>)/is", "$1".$gtmjoomla_iframe_container_code, $buffer, 1);
